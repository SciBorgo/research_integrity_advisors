...
bplot = ggplot(data = numbers, aes(x=n))+
  geom_bar(fill='dark orange', col='grey55')+
  g.theme+
...
hplot = ggplot(data = all_boot, aes(x = total))+
  geom_histogram(fill='goldenrod1', col = 'grey55')+
  g.theme+
  ylab('Count')+
...
The estimated median number of national advisors is `r median(all_boot$total)` with 1st quartile of `r quantile(all_boot$total, 0.25)` and 3rd quartile of `r quantile(all_boot$total, 0.75)`.
...
```{r}
# made an inset because of the strong skew in the plot
inset = ggplot(data = data, aes(x=q3_3)) +
  geom_histogram(fill = sample_colours[1], col='grey22', binwidth=0.2)+
...
results = make_table_matrix(indata = data, 
                            ...
                            ordered = TRUE, 
                            ordered_row = 'Agree', 
                            legend =  c('Agree','Neither agree nor disagree','Disagree','Don`t know') ,
                            bar_colours = bar_colours, 
...
bar = ggplot(data=data, aes(x = start_date))+
  geom_bar(position='stack', fill=bar_colours[1], col='grey88')+
  g.theme +
  xlab('')+
  ylab('Frequency')
bar
